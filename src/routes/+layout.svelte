	...
	<header class="-order-1 flex flex-col py-4 px-6 gap-6 bg-slate-200 lg:w-80">
		...
		<nav class="flex-col text-slate-600 gap-6 flex lg:flex" class:hidden={navHidden}>
			<ul class="flex flex-col gap-2">
				{#each headerLinks as { text, href, icon }}
					<li>
						<a
							{href}
			...
			<ul class="flex flex-col gap-2">
				{#each socialLinks as { text, href, icon }}
					<li>
						<a {href} rel="me" class="font-heading flex items-center gap-2">
							{#if icon}
								<Icon name={icon} class="w-4 h-4 shrink-0" />
							{/if}
							<span>{text}</span>
			...
			<ul class="flex flex-col gap-2">
				{#each footerLinks as { text, href, icon }}
					<li>
						<a
							{href}
							class="font-heading flex items-center gap-2"
							data-sveltekit-reload={href === '/rss.xml'}
						>
							{#if icon}
								<Icon name={icon} class="w-4 h-4 shrink-0" />
							{/if}
